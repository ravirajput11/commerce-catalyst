...
const TanyaShoppingAssistant = () => {
  ...
  const [inputText, setInputText] = useState(""); // Separate input state
  ...
  const cleanResponse = (html: string) =>
    decode(html).replace(/<span class=['"][^'"]*['"]>(.*?)<\/span>/g, "$1");
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const URL = `http://localhost:5000/api/web-bff/assistant`;
      ...
      setChatHistory((prev) =>
        prev.map((msg, idx) =>
          idx === prev.length - 1
            ? { ...msg, response: cleanResponse(res.data.response) }
            : msg
        )
      );
    } catch (error) {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="absolute bottom-6 -right-4 w-96 h-96 bg-white p-0 rounded-xl">
        ...
        <div
          ref={scrollRef}
          className="h-[calc(100vh-300px)] overflow-y-auto pr-5 space-y-4 hide-scrollbar"
        >
          ...
          {chatHistory.map((chat, index) => (
            <div key={index}>
              <p className=" text-sm text-[#000000] bg-[#F1DCFF] drop-shadow-md p-3 m-3 rounded-r-xl rounded-bl-2xl w-3/4">
                {chat.query}
              </p>
              {chat.response && (
